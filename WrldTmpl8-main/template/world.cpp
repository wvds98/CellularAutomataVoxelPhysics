...
World::World( const uint targetID )
{
	...
	uint bs2 = bs / 4;
	brick = (PAYLOAD*)_aligned_malloc( bs, 64 );
#if ONEBRICKBUFFER == 1
	brickBuffer = new Buffer( bs2 /* dwords */, Buffer::DEFAULT, (uchar*)brick );
...
//
void SetWorldVoxelsFromPhysics()
{
...
void World::DrawTileVoxels( const uint cellIdx, const PAYLOAD* voxels, const uint zeroes )
{
	...
	uint brickIdx;
	if ((g & 1) == 1) brickIdx = g >> 1; else brickIdx = g >> 1, grid[cellIdx] = (brickIdx << 1) | 1;
