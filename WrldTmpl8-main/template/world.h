...
public:
	...
	__forceinline uint Get( const uint x, const uint y, const uint z )
	{
		...
		uint bx = (x / BRICKDIM) & (GRIDWIDTH - 1);
		uint by = (y / BRICKDIM) & (GRIDHEIGHT - 1);
		uint bz = (z / BRICKDIM) & (GRIDDEPTH - 1);

		const uint cellIdx = bx + bz * GRIDWIDTH + by * GRIDWIDTH * GRIDDEPTH;
		const uint g = grid[cellIdx]; //Get brick index
		if (!(g & 1)) return 0; //empty brick

		// calculate the position of the voxel inside the brick and return that voxel
	...
	__forceinline void Set( const uint x, const uint y, const uint z, const uint v /* actually an 8-bit value */ )
	{
		...
		//Gain or lose a zero
		int zeroChange = (cv != 0 && v == 0) - (cv == 0 && v != 0);
